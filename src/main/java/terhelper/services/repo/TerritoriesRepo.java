...
public class TerritoriesRepo {
    ...
    public boolean removeAllLocationsByTerritory(String territoryId) {
        try {
            ...
            System.out.println(url.build());
            CloseableHttpResponse response = api.sendRequest(url, "DELETE");
            if (null != response) {
                System.out.println(response.getStatusLine().toString());
            } else {
                System.out.println("nullable response");
            }
        } catch (Exception ex) {
